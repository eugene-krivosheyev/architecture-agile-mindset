...
- Можем запроектировать классический "тупой" веб-клиент. Тогда в случае нарушения канала от клиента к серверам, пользователю недоступен никакой функционал. То есть мы пожертвовали характеристикой Partitioning tolerance, и система целиком не работает в этом случае. Зато если канал работает, то пользователь получает функционал быстро и данные согласованы по всей системе.
- Можем запроектировать "умный" клиент с хранением данных и поведением на стороне пользователя, например, мобильное приложение. Тогда это приложение работоспособно даже в условиях нарушения канала от клиента к серверам (не жертвуем P). И придется выбирать между A и C: или пользователь может совершать операции в мобильном приложении (система Доступна) ценой расхождения данных в системе, Нецелостности этих данных (в мобильном приложении после операции одни данные, а на серверах – другие и после восстановления канала данные синхронизируется, но система в целом долгое время находится в рассогласованном состоянии).
