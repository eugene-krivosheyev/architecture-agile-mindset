...
1. Когда будешь читать каталоги паттернов, попробуй для себя переформулировать их в такой форме:
```
Чтобы <задача>,
Я распределяю ответственности по модулям таким образом: <решение>,
Что изменю <как поменяются характеристики системы> ценой <trade-off>.
```
